...
function mapStateToProps(state) {
  return {
    ...
    isSpotifyShowingArtistDetails: state.spotifyArtistDetails.get(
  ...
  return bindActionCreators(
    {
      ...
      setSpotifyIsShowingArtistDetails:
        SpotifyArtistDetails.setIsShowingArtistDetails,
...
const sliders = ["popularity", "acousticness", "danceability"];

const zeroToOneSliders = ["acousticness", "danceability"];
...
export const App = React.createClass({
  getInitialState() {
    const sliderValues = {};
    sliders.forEach(
      slider => (sliderValues[`${spotifyPropertyPrefix}_${slider}`] = 0)
    );
    sliderValues.tempo = 0;
    return {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.isSpotifyAuthenticated &&
  ...
  render() {
    if (
      !this.props.spotifySelectedArtist.size ||
      !this.props.isSpotifyAuthenticated
    ) {
      return null;
    }
    ...
    const shouldRenderLoading =
      this.props.spotifyRecs.size && this.props.isFetchingSpotifyRecs;
    ...
    return (
      <div className="app-container">
        ...
        <div className="fixed-top bg-orange">
          <div className="section-main u-ph- u-pb-">{this.renderSearch()}</div>
        </div>

        <div className="u-pt+">
          {this.props.spotifyRecs.size
            ? this.renderSpotifyRecs()
  ...
  renderInitialScreen() {
    return (
      <div className="initial-screen u-pt+">
    ...
    return (
      <div className="section-main u-ph-">
        <div className="layout">
          {!isPalm && (
            <div className="layout__item u-1/4 u-2/5-lap u-1/1-palm">
              {this.renderSidebar()}
            </div>
          )}
  ...
  renderSidebar() {
    return (
      <div className="sidebar u-pt">
        <div
          className="drawer-header"
          onClick={this.toggleSpotifySlidersDrawer}
        >
          Advanced Settings
        </div>
        <Drawer
          isOpen={this.state.isSpotifySlidersDrawerOpen}
          onToggleCallback={this.onPlaylistDrawerToggle}
        >
          {this.renderSliders()}
          {this.renderBPMSlider()}
        </Drawer>

        <Waypoint
          offsetTop={60}
          reset={this.state.playlistDrawerReset}
          isDisabled={this.props.device !== "desk"}
        >
          <div
            className="drawer-header"
            onClick={this.toggleSpotifyPlaylistsDrawer}
          >
            My playlists
          </div>
          <Drawer
            isOpen={this.state.isSpotifyPlaylistsDrawerOpen}
            onToggleCallback={this.onPlaylistDrawerToggle}
          >
            {this.props.isSpotifyUserAuthenticated
              ? this.renderUserPlaylists()
              : this.renderSpotifyLoginButton()}
          </Drawer>
        </Waypoint>
      </div>
    );
  ...
  renderSpotifyLoginButton() {
    return [
      ...
      <button
        key="1"
        className="btn btn--small btn--pill u-1/1"
        onClick={() => window.open("/api/v1/spotify-login")}
      >
  ...
  renderUserPlaylists() {
    ...
    const ownedPlaylists = playlists.filter(
      playlist => playlist.getIn(["owner", "id"]) === userId
    );
    return (
      <div>
  ...
  renderTracks() {
    ...
    return (
      <div
        className={`spotify-tracks layout ${
          isPalm ? "layout--small" : ""
        } u-mt u-mb++`}
      >
        {this.props.spotifyRecs
          .get("tracks")
          .map(track => this.renderTrack(track))}
      </div>
    );
  },
  ...
  renderTrack(track) {
    return (
      <div key={track.get("id")} className="layout__item u-1/5 u-1/3-portable">
        <div className="u-mb">
          <SpotifyTrack
            {...this.props}
            track={track}
            onTrackClick={this.openTrackDetails}
          />
        </div>
      </div>
    );
  ...
  renderTrackDetails() {
    ...
    return (
      <Modal onClose={this.closeTrackDetails}>
        <div className="master-detail">
          <div className="master-detail__sidebar">
            {artistImg ? (
              <ImgLoader className="u-250px" src={artistImg} />
            ) : null}
            <h3 className="u-pl-">{spotifyArtistDetails.get("name")}</h3>
          </div>
  ...
  renderAlbum(album) {
    ...
    return (
      <li onClick={e => this.loadAlbum(id)} key={id} className="text-truncate">
  ...
  renderAlbumTracks(tracks) {
    return (
      <Drawer isOpen={true} shouldAnimateInitialOpen={true}>
        <ul className="list-bare u-pv- u-pl bg-white">
          {tracks.map(track => (
            <li
              key={track.get("id")}
              onMouseEnter={() =>
                this.refs[`${track.get("id")}-preview`].play()
              }
              onMouseLeave={() =>
                this.refs[`${track.get("id")}-preview`].pause()
              }
            >
              {track.get("name")}
              <audio
                className="spotify-track__preview"
  ...
  renderSearch() {
    return (
      <div className="search u-pt-">
        <div className="layout layout--flush">
          <div className="layout__item u-1/4">
            <div className="text-center u-pr-- palm-pt--">
              <img
                onClick={e => this.props.push("/")}
                className="logo block u-1/1"
                src="/static/img/logo_spotworm_white.svg"
              ></img>
            </div>
          </div>
  ...
  renderSliders() {
    return (
      <div className="sliders u-ph-- u-pv-">
        {sliders.map((slider, index) =>
          this.renderSlider(`${spotifyPropertyPrefix}_${slider}`, index)
        )}
      </div>
    );
  },
  ...
  renderBPMSlider() {
    return (
      <div>
        <label>bpm {this.getSliderVal("tempo")}</label>
        <RangeSlider
          min={0}
          max={200}
          step={1}
          value={this.state.targets["tempo"]}
          onChange={val => this.setSliderVal("tempo", val)}
        />
      </div>
    );
  },
  ...
  toggleSpotifySlidersDrawer() {
    this.setState({
      isSpotifySlidersDrawerOpen: !this.state.isSpotifySlidersDrawerOpen
    });
  },

  toggleSpotifyPlaylistsDrawer() {
    this.setState({
      isSpotifyPlaylistsDrawerOpen: !this.state.isSpotifyPlaylistsDrawerOpen
    });
  },
  ...
  fetchRecs(artistId) {
    ...
    if (this.state.targets.tempo) {
      params.target_tempo = this.state.targets.tempo;
    }
